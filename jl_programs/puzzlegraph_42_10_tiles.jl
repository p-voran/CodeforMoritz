...
output_file = open("output_file_42_10_tiles.text", "w")


#sparse starting configurations
sparse_start42_10tiles= [SVector{10, UInt8}(0,1,2,4,8,11,12,13,14,15)]
...
function calculateConfigurations(start, facecollection)
    ...
    res::Vector{SVector{10, UInt8}} = copy(start)
    to_do::Vector{SVector{10, UInt8}} = copy(start)
    previous::Vector{SVector{10, UInt8}} = copy(start)
    counter = 0
    while true
        if length(to_do) == 0
...
function calculate_one_step(previous, facecollection)
    next_step::Vector{SVector{10, UInt8}} = []
            ...
            for k_face in facecollection[position+1]
                if is_k_face_empty(configuration, k_face)
                    ...
                    for corner in k_face
                        push!(next_step, SVector{10, UInt8}(setindex!([x for x in configuration], corner, label)))
                    end
                end
            end
        end
...
test = @time calculateConfigurations(sparse_start42_10tiles, facecollection42)

if SVector{10, UInt8}(1,0,2,4,8,11,12,13,14,15) in test
